...
(async()=>{
 ...
 const ctx=vm.createContext({document:{addEventListener(){},querySelector:node},window:{addEventListener(){}},location:{pathname:'/er/items'},loadAssetConfig:async()=>{},erDictionary:async()=>new Map(),erStatic:url=>url.includes('statistics')?new Promise(r=>finishStats=r):Promise.resolve({data:url.endsWith('weapon')?[{code:1,name:'검',weaponType:'Glove'}]:[]}),erText:String,erNumber:v=>v??'—',erGradeNames:{},erItemHtml:()=>'',erApplyItemGrades(){}});
 ...
 await Promise.race([ctx.erItemsPage(),new Promise((_,reject)=>setTimeout(()=>reject(Error('statistics blocked items')),500))]);
 ...
 console.log('PASS: item list usable with indefinitely pending statistics');
