...
async function erItemsPage() {
    ...
    root.innerHTML='<aside class="database-sidebar surface item-filters"><h2>아이템 필터</h2><button type="button" id="item-all" aria-pressed="true">모든 아이템</button><div id="item-type-groups"></div><label>등급<select id="item-grade"><option value="">전체 등급</option>'+Object.entries(erGradeNames).map(([key,name])=>'<option value="'+key+'">'+name+'</option>').join('')+'</select></label></aside><section class="surface item-table-panel"><div class="item-search-bar"><input id="item-query" aria-label="아이템 검색" placeholder="아이템 이름 / 코드"></div><div id="item-list"></div><button id="more-items">더 보기</button></section><aside id="item-detail" class="surface item-detail"><p class="empty-state">아이템을 선택해 주세요.</p></aside>';
...
async function erRankingPage() {
    ...
    const pump=()=>{while(active<2&&queue.length){const uid=queue.shift();if(!Array.from(root.querySelectorAll('[data-rank-id]')).some(tr=>tr.dataset.rankId===uid)){requested.delete(uid);continue;}active++;
        erRequest('/er/userRank?userNum='+encodeURIComponent(uid)).then(data=>{const stats=data.userStats?.[0]||{};profiles.set(uid,stats);try{sessionStorage.setItem('ergg.rank.profile.'+uid,JSON.stringify({at:Date.now(),stats}));}catch(_){}return paint(uid,stats);}).catch(()=>{requested.delete(uid);root.querySelectorAll('[data-rank-id]').forEach(tr=>{if(tr.dataset.rankId===uid)tr.querySelector('.rank-character-top').innerHTML='<span class="data-note">통계 조회 실패</span>';});}).finally(()=>{active--;setTimeout(pump,350);});
    }};
    const observer=new IntersectionObserver(entries=>{for(const entry of entries){if(!entry.isIntersecting)continue;observer.unobserve(entry.target);const uid=entry.target.dataset.rankId;if(!uid)continue;
        if(!profiles.has(uid)){try{const cached=JSON.parse(sessionStorage.getItem('ergg.rank.profile.'+uid)||'null');if(cached&&Date.now()-cached.at<900000)profiles.set(uid,cached.stats);}catch(_){}}
