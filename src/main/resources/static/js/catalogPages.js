...
async function erRankingPage() {
    ...
    const pump=()=>{while(active<2&&queue.length){const uid=queue.shift();if(!Array.from(root.querySelectorAll('[data-rank-id]')).some(tr=>tr.dataset.rankId===uid)){requested.delete(uid);continue;}active++;
        Promise.resolve().then(async()=>{const row=Array.from(root.querySelectorAll('[data-rank-id]')).find(tr=>tr.dataset.rankId===uid);let userId=row?.dataset.userId;if(!userId){const found=await erStatic('/er/search/nickname?nickname='+encodeURIComponent(uid));userId=found.user?.userId;}if(!userId)throw new Error('플레이어 ID 없음');return erRequest('/er/userRank?userNum='+encodeURIComponent(userId));}).then(data=>{const stats=data.userStats?.[0]||{};profiles.set(uid,stats);try{sessionStorage.setItem('ergg.rank.profile.'+uid,JSON.stringify({at:Date.now(),stats}));}catch(_){}return paint(uid,stats);}).catch(()=>{requested.delete(uid);root.querySelectorAll('[data-rank-id]').forEach(tr=>{if(tr.dataset.rankId===uid)tr.querySelector('.rank-character-top').innerHTML='<span class="data-note">통계 조회 실패</span>';});}).finally(()=>{active--;setTimeout(pump,350);});
    }};
