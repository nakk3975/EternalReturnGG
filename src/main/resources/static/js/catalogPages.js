...
function erRoutesMarkup(routes,names) {
    if(!routes.length)return '<p class="empty-state">현재 제공되는 추천 루트가 없습니다.</p>';
    return '<div class="route-list">'+routes.map(r=>
        '<article class="route-list-row">'+
        '<div class="route-list-info"><div class="route-list-heading">'+
        '<span class="route-number">#'+erText(r.id)+'</span>'+
        '<button type="button" class="copy-route" data-route-id="'+erText(r.id)+'" aria-live="polite">번호 복사</button></div>'+
        '<h3>'+erText(r.title)+'</h3><p>'+erText(r.userNickname)+'</p></div>'+
        '<div class="route-list-items"><span>아이템 빌드</span><div class="item-slots">'+
        (String(r.weaponCodes||'').match(/\d{6}/g)||[]).slice(0,5).map(erItemHtml).join('')+'</div></div>'+
        '<a class="sim-route-link" href="/er/route-planner?routeId='+erText(r.id)+'">시뮬레이터에서 열기</a></article>'
    ).join('')+'</div>';
}
