...
async function startRoutePlanner(){
 ...
 const optionalMetadata=Promise.allSettled([erStatic('/er/tacticalSkill'),erStatic('/er/trait'),erStatic('/er/route-data/CharacterMastery'),erStatic('/er/route-data/CharacterLevelUpStat'),erStatic('/er/route-data/CreditShop')]);
 const [_,weapons,armor,misc,names,areaBody,spawnBody,gatherBody,collectBody,consumables,special,characters]=await Promise.all([loadAssetConfig(),erStatic('/er/weapon'),erStatic('/er/armor'),erStatic('/er/materials'),erDictionary(),erStatic('/er/route-data/Area'),erStatic('/er/route-data/ItemSpawn'),erStatic('/er/route-data/NaviCollectAndHunt'),erStatic('/er/route-data/Collectible'),erStatic('/er/route-data/ItemConsumable'),erStatic('/er/route-data/ItemSpecial'),erStatic('/er/character')]);
 ...
 optionalMetadata.then(values=>{
  ...
  tactical.insertAdjacentHTML('beforeend',(tacticals.data||[]).filter(erIsAvailableTactical).map(r=>'<option value="'+erText(r.group)+'">'+erText(names.get('Skill/Group/Name/'+String(r.icon||'').match(/(\d+)$/)?.[1])||r.name||r.group)+'</option>').join(''));
