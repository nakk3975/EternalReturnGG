...
if(!isSettings){desktop.onSnapshot(renderHud);desktop.snapshot().then(renderHud);}
 ...
 const active=new Set(Object.keys(erAnimals));
 ...
 function publish(){const now=erHuntSeconds(time);route=route.filter(c=>active.has(c.type)&&erHuntAvailable(c,time));
  ...
  $('clock-label').textContent=data.clock;document.querySelector('#map svg').innerHTML=mapSvg(data);desktop.publish(data);renderHud(data);
 }
 ...
 $('settings').addEventListener('click',e=>{if(e.target.dataset.action)desktop.action(e.target.dataset.action).catch(e=>notice(e.message));});
 $('opacity').oninput=()=>desktop.action('opacity',Number($('opacity').value)/100);
 $('team-form').onsubmit=async e=>{e.preventDefault();const names=$('names').value.split(',').map(s=>s.trim()).filter(Boolean);if(!names.length||names.length>3)return notice('닉네임 1~3명을 쉼표로 구분해 입력하세요.');const button=e.target.querySelector('button');button.disabled=true;notice('팀 정보 조회 중 · 서버 첫 접속은 시간이 걸릴 수 있습니다.');try{const rows=await desktop.team(names);team=rows.map(p=>({nickname:p.nickname,error:p.error,rank:p.stats?(erTier(p.stats)?.name||'랭크 기록 없음')+' · '+erNumber(p.stats.mmr)+' RP':'랭크 기록 없음'}));publish();notice('팀 정보 조회 완료 · 오류가 있는 팀원은 다시 조회하세요.');}catch(err){notice(err.message);}finally{button.disabled=false;}};
 $('sources').onclick=async()=>{try{const sources=await desktop.sources();$('source').replaceChildren(new Option('공유할 창 선택',''));sources.forEach(s=>$('source').add(new Option(s.name,s.id)));await desktop.selectSource('');}catch(e){notice(e.message);}};
 $('source').onchange=()=>desktop.selectSource($('source').value).catch(e=>notice(e.message));desktop.onWarning(notice);
 ...
 setInterval(()=>{if(running){const total=erHuntPhases.reduce((a,b)=>a+b,0),now=Math.min(total,base+Math.floor((performance.now()-baseAt)/1000));time=erHuntAt(now);if(now===total)pause();syncInputs();}publish();},1000);
